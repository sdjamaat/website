...
const ChooseFamily = ({ layout, setStep, values, setValues, getFamilies }: any) => {
  const [form] = Form.useForm()
  const [familyIndex, setFamilyIndex] = useState(values.familyindex)
  const [familiesFromRegisterComponent, setFamiliesFromRegisterComponent] = useState<any[] | null>(null)
  ...
  const onFinish = (values: any) => {
    if (!values.hasOwnProperty("memberindex")) {
      CustomMessage("error", "Cannot move forward without selecting an eligible family member")
    } else if (values.memberindex >= familiesFromRegisterComponent![values.familyindex].members.length) {
  ...
  const handleFamilySelection = (value: number) => {
      ...
      >
        {familiesFromRegisterComponent !== null ? (
          <Form.Item
            ...
            name="familyindex"
            rules={[{ required: true, message: "Please input family affiliation" }]}
          >
            <Select onChange={handleFamilySelection}>
              {familiesFromRegisterComponent.map((family, index) => (
                <Option key={index} value={index}>{family.displayname}</Option>
              ))}
            </Select>
          </Form.Item>
        ) : (
        ...
        {shouldShowMemberSelection() ? (
          <Form.Item
            label="Member"
            name="memberindex"
            rules={[{ required: true, message: "Please input family member selection" }]}
          >
            <Select>
              {familiesFromRegisterComponent![familyIndex].members.map(
                (member: any, index: number) => {
                  if (member.uid === null) {
                    return (
                      <Option key={index} value={index}>{member.firstname}</Option>
                    )
                  } else {
                    return null
                  }
